...
import { ListeningTestCard } from './components';
import {
...
import {
  LISTENING_LIST_DEFAULT_PAGE_SIZE,
  findLatestUnfinishedListeningExamForTest,
  getListeningActiveExamId,
  setListeningActiveExam,
} from './api/utils';
import { ListeningCatalogSkeleton } from './skeleton';

export default function AppsListeningCatalogView() {
  const { tx } = useLocales();
  const router = useRouter();
  const [startingTestId, setStartingTestId] = useState<number | null>(null);

  const listState = useUrlListState({
    defaultPageSize: LISTENING_LIST_DEFAULT_PAGE_SIZE,
    defaultOrdering: 'created_at',
  });

  const listQuery = useListeningListQuery({
    page: listState.page,
    rowsPerPage: LISTENING_LIST_DEFAULT_PAGE_SIZE,
  });
  const examsQuery = useMyListeningExamsQuery();
  ...
  const handleStart = async (testId: number, examId?: number | null) => {
    ...
    try {
      ...
      router.push(paths.ielts.listeningSession(String(testId)));
    } finally {
  ...
  return (
    <Container maxWidth="lg">
      ...
      {showInitialSkeleton ? <ListeningCatalogSkeleton /> : null}
            ...
            >
              <Stack spacing={0.75}>
                <Typography variant="subtitle1" sx={{ color: 'primary.darker', fontWeight: 700 }}>
                  {tx('pages.ielts.listening.title')}
                </Typography>
          ...
          {listQuery.data.items.length ? (
            <Grid container spacing={3}>
              {listQuery.data.items.map((item) => {
                const storedActiveExamId = getListeningActiveExamId(item.id);
                const latestActiveExam =
                  findLatestUnfinishedListeningExamForTest(item.id, exams) ?? null;
                const attemptHistoryItems = toModuleAttemptHistoryItems('listening', exams, item.id);
                      ...
                      actions={
                        <Stack spacing={1}>
                          <Button
                            ...
                            variant="outlined"
                            color="primary"
                            onClick={() => router.push(paths.ielts.listeningTest(String(item.id)))}
                          >
