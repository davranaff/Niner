...
} from '../api/listening-requests';
import type { BackendListeningExamResultStatus } from '../api/types';
...
function resolveFinishReason(
  ...
  resultStatus: BackendListeningExamResultStatus
) {
...
export default function AppsListeningResultView() {
  const { tx } = useLocales();
  ...
  const resultQuery = useFetch(
    ['student-listening-attempt-result', examId],
    () => fetchListeningExamResult(examId),
    { enabled: examId > 0 }
  );
  const myExamsQuery = useFetch(
    ['student-listening-attempts-context', examId],
    () => fetchMyListeningExams({ listeningOffset: 0, limit: 100 }),
    { enabled: examId > 0 }
  );

  const listeningExam = useMemo(
    () =>
      myExamsQuery.data?.listening.items.find(
        (item) => item.kind === 'listening' && item.id === examId
      ) ?? null,
    [examId, myExamsQuery.data?.listening.items]
  );

  const detailQuery = useFetch(
    ['student-listening-attempt-test-detail', listeningExam?.testId],
    () => fetchListeningDetail(Number(listeningExam?.testId)),
    { enabled: Boolean(listeningExam?.testId) }
  );
        ...
        <EmptyContent
          ...
          action={
            <Button
              variant="contained"
              color="inherit"
              onClick={() => router.push(paths.ielts.listening)}
            >
              {tx('pages.ielts.shared.return_to_listening')}
            </Button>
          }
          sx={{ py: 10 }}
        />
  ...
  const finishReason = resolveFinishReason(listeningExam?.finishReason, result.result);
  const totalQuestions = detail?.parts?.reduce((total, part) => total + part.questionsCount, 0) ?? null;
  ...
  return (
    <Container maxWidth="lg">
      <AppsPageHeader
        ...
        description={detail?.title ?? `Listening attempt #${examId}`}
        action={
          <Stack direction="row" spacing={1}>
            {detail ? (
              <Button
                variant="outlined"
                color="inherit"
                onClick={() => router.push(paths.ielts.listeningTest(String(detail.id)))}
              >
                {tx('pages.ielts.shared.details')}
              </Button>
            ) : null}

            <Button variant="outlined" color="inherit" onClick={() => router.push(paths.ielts.listening)}>
              {tx('pages.ielts.shared.return_to_listening')}
            </Button>
          </Stack>
        }
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} md={3}>
              ...
              <Typography variant="subtitle2">
                {listeningExam?.finishedAt || listeningExam?.startedAt
                  ? fDateTime(listeningExam?.finishedAt || listeningExam?.startedAt)
                  : '-'}
              </Typography>
            </Grid>
          </Grid>
      ...
      {isLeftTerminated ? (
        ...
        <AttemptAiSummaryCard module="listening" examId={examId} />
      )}

      <PostExamAssignmentsCard module="listening" examId={examId} />
                    ...
                    >
                      <Stack spacing={0.5}>
                        <Typography variant="subtitle2" sx={{ color: 'text.primary' }}>
                          {tx('pages.ielts.shared.task_label', { number: part.partNumber })}
                        </Typography>
                        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                          {part.title}
                        </Typography>
                      ...
                      <Stack alignItems={{ xs: 'flex-start', sm: 'flex-end' }} spacing={0.5}>
                        ...
                        <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                          {totalQuestions
                            ? `${Math.round((part.questionsCount / totalQuestions) * 100)}%`
                            : '-'}
                        </Typography>
                      </Stack>
                    </Stack>
