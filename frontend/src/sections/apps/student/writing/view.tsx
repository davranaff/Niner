...
import { WritingTestCard } from './components';
import { WritingCatalogSkeleton } from './skeleton';

export default function AppsWritingCatalogView() {
  const { tx } = useLocales();
  const router = useRouter();
  const [startingTestId, setStartingTestId] = useState<number | null>(null);

  const listState = useUrlListState({
    defaultPageSize: WRITING_LIST_DEFAULT_PAGE_SIZE,
    defaultOrdering: 'created_at',
  });

  const listQuery = useWritingListQuery({
    page: listState.page,
    rowsPerPage: WRITING_LIST_DEFAULT_PAGE_SIZE,
  });
  const examsQuery = useMyWritingExamsQuery();
  ...
  const handleStart = async (testId: number, examId?: number | null) => {
    ...
    try {
      ...
      router.push(paths.ielts.writingSession(String(testId)));
    } finally {
  ...
  return (
    <Container maxWidth="lg">
      ...
      {showInitialSkeleton ? <WritingCatalogSkeleton /> : null}
